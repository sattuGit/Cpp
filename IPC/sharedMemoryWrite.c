...
int main(){
    ...
    for (int msgIndex = 0; msgIndex < 3; ++msgIndex) {
        // reset buffer 
        pBuf = pShm->buffer; 
        memset(pShm->buffer,'\0',MSG_SIZE);    
        
        // filling shared memeory 
        fgets(pBuf,MSG_SIZE-1,stdin);
        pShm->len = strlen(pBuf);
        pShm->isDone = false;
    ...
    /**/
    if(shmctl(shareMemDesc,IPC_RMID,0) == -1){
        perror("shmctl");
        return EXIT_FAILURE;
    }
